"""
RL Joint Optimizer: CPU × CPU_freq × DDR × LLCC × DDRQOS
===========================================================
Finds the best combination of ALL frequency dimensions for minimum DRAM latency.

Action space: 241 × 11 × 9 × 2 = 47,718 combinations
  CPU × CPU_freq : 241  (8 CPUs × available frequencies)
  DDR            : 11   (547 MHz to 5333 MHz)
  LLCC           : 9    (282 MHz to 1350 MHz)
  DDRQOS         : 2    (0 or 1)

Frequency locking approach:
  Setup:    All bus_dcvs nodes (bwmon + memlat) set to hw_min
  Per run:  bwmon min=max=target (DDR/LLCC), memlat prime min=max=target (DDRQOS)
            → these become the only active votes above hw_min → win

Usage:
    python rl_benchmark.py                    # 500 episodes, 40MB buffer
    python rl_benchmark.py --episodes 1000
    python rl_benchmark.py --buffer-kb 65536

Requirements:
    pip install numpy
"""

import subprocess
import re
import time
import numpy as np
import json
import argparse

# ---------------------------------------------------------------------------
# Device CPU map
# taskset mask is HEX: taskset 80 = 0x80 = CPU7
# Thermal zones verified on device via grep -r cpu /sys/class/thermal/*/type
# ---------------------------------------------------------------------------
_TZ = '/sys/class/thermal/thermal_zone{}/temp'
CPU_INFO = {
    0: {'mask': '1',  'cluster': 'LITTLE', 'thermal': _TZ.format(28)},  # cpu-0-0
    1: {'mask': '2',  'cluster': 'LITTLE', 'thermal': _TZ.format(31)},  # cpu-1-0
    2: {'mask': '4',  'cluster': 'LITTLE', 'thermal': _TZ.format(10)},  # cpu-2-0
    3: {'mask': '8',  'cluster': 'Gold',   'thermal': _TZ.format(0)},   # cpu-3-0
    4: {'mask': '10', 'cluster': 'Gold',   'thermal': _TZ.format(14)},  # cpu-4-0
    5: {'mask': '20', 'cluster': 'Gold',   'thermal': _TZ.format(2)},   # cpu-5-0
    6: {'mask': '40', 'cluster': 'Prime',  'thermal': _TZ.format(18)},  # cpu-6-0
    7: {'mask': '80', 'cluster': 'Prime',  'thermal': _TZ.format(6)},   # cpu-7-0
}

CLUSTER_FREQS = {
    'LITTLE': [864000, 960000, 1056000, 1152000, 1248000, 1344000, 1440000,
               1536000, 1632000, 1728000, 1824000, 1920000, 2016000, 2112000,
               2208000, 2304000, 2400000, 2496000, 2592000, 2707200, 2822400,
               2937600, 3052800, 3168000, 3283200, 3398400, 3475200],
    'Gold':   [960000, 1056000, 1152000, 1248000, 1344000, 1440000, 1536000,
               1632000, 1728000, 1824000, 1920000, 2016000, 2112000, 2208000,
               2304000, 2400000, 2496000, 2592000, 2707200, 2822400, 2937600,
               3052800, 3168000, 3283200, 3398400, 3513600, 3628800, 3744000,
               3859200, 3916800],
    'Prime':  [288000, 384000, 518400, 576000, 672000, 768000, 864000, 960000,
               1056000, 1152000, 1248000, 1344000, 1440000, 1536000, 1632000,
               1728000, 1824000, 1920000, 2016000, 2112000, 2208000, 2400000,
               2630400, 2860800, 3091200, 3321600, 3552000, 3667200, 3782400,
               3897600, 4012800, 4128000, 4243200, 4358400, 4435200],
}

DDR_FREQS   = [547000, 1353000, 1555000, 1708000, 2092000, 2736000,
               3187000, 3686000, 4224000, 4780000, 5333000]
LLCC_FREQS  = [282000, 350000, 533000, 600000, 806000, 933000, 1066000, 1211000, 1350000]
DDRQOS_VALS = [0, 1]

ACTIONS = [
    (cpu, cf, d, l, q)
    for cpu, info in CPU_INFO.items()
    for cf in CLUSTER_FREQS[info['cluster']]
    for d in DDR_FREQS
    for l in LLCC_FREQS
    for q in DDRQOS_VALS
]
N_ACTIONS = len(ACTIONS)   # 47,718

DEFAULT_BUF  = 40960
DEFAULT_TEMP = 30

# bwmon device paths (verified writable after adb root)
DDR_BWMON    = "/sys/devices/system/cpu/bus_dcvs/DDR/31081000.qcom,bwmon-ddr"
LLCC_BWMON   = "/sys/devices/system/cpu/bus_dcvs/LLCC/310b7500.qcom,bwmon-llcc"
DDRQOS_PRIME = "/sys/devices/system/cpu/bus_dcvs/DDRQOS/soc:qcom,memlat:ddrqos:prime"
DDRQOS_BASE  = "/sys/devices/system/cpu/bus_dcvs/DDRQOS"


# ---------------------------------------------------------------------------
# ADB helpers
# ---------------------------------------------------------------------------
def _adb(cmd, timeout=15):
    subprocess.run(['adb', 'shell', cmd], capture_output=True, timeout=timeout)


def _lock_node(path, freq):
    """Set min=max=freq on a sysfs node (max first for ordering safety, repeated twice)."""
    _adb(f"echo {freq} > {path}/max_freq")
    _adb(f"echo {freq} > {path}/min_freq")
    _adb(f"echo {freq} > {path}/max_freq")
    _adb(f"echo {freq} > {path}/min_freq")


def _read_hw_min(bus):
    try:
        r = subprocess.run(
            ['adb', 'shell', f'cat /sys/devices/system/cpu/bus_dcvs/{bus}/hw_min_freq'],
            capture_output=True, text=True, timeout=10)
        v = r.stdout.strip()
        return int(v) if v.isdigit() else 0
    except Exception:
        return 0


# ---------------------------------------------------------------------------
# Device setup
# ---------------------------------------------------------------------------
def discover_and_silence_all():
    """
    Discover all bus_dcvs nodes via:
      ls -d /sys/devices/system/cpu/bus_dcvs/*/*qcom*
    Set ALL nodes (bwmon + memlat) to hw_min.
    Per-episode writes then become the only active votes above hw_min → win.
    """
    result = subprocess.run(
        ['adb', 'shell', 'ls -d /sys/devices/system/cpu/bus_dcvs/*/*qcom*'],
        capture_output=True, text=True, timeout=15)
    paths = [p.strip() for p in result.stdout.splitlines() if p.strip()]

    hw_min = {b: _read_hw_min(b) for b in ('DDR', 'DDRQOS', 'LLCC')}
    print(f"  [SETUP] hw_min: DDR={hw_min['DDR']} DDRQOS={hw_min['DDRQOS']} LLCC={hw_min['LLCC']}")

    count = 0
    for path in paths:
        bus = next((b for b in ('DDR', 'DDRQOS', 'LLCC') if f'/bus_dcvs/{b}/' in path), None)
        if bus is None:
            continue
        _lock_node(path, hw_min[bus])
        kind = "bwmon " if 'bwmon' in path else "memlat"
        print(f"  [SETUP]   {kind} {path.split('/')[-1]} → {hw_min[bus]}")
        count += 1

    print(f"  [SETUP] {count} nodes set to hw_min")


def setup_device():
    print("  [SETUP] Rooting...")
    subprocess.run(['adb', 'root'], capture_output=True, timeout=15)
    subprocess.run(['adb', 'wait-for-device'], capture_output=True, timeout=30)

    print("  [SETUP] Pushing benchmark binary...")
    subprocess.run([
        'adb', 'push',
        r'\\sundae\APT_Logs_PPTKGolden\Kernel\Test-binaries-compact\full-rand-lat\FullRandLat',
        '/data/local/tmp/'
    ], capture_output=True, timeout=60)
    _adb("chmod 777 /data/local/tmp/*")

    print("  [SETUP] Waking screen...")
    _adb("input keyevent 82")
    _adb("input keyevent 82")
    _adb("input keyevent 82")
    _adb("input keyevent 3")
    _adb("settings put system screen_off_timeout 2147483647")

    discover_and_silence_all()
    print("  [SETUP] Done")


# ---------------------------------------------------------------------------
# Frequency control
# ---------------------------------------------------------------------------
def set_cpu_freq(cpu_id, freq_hz):
    path = f"/sys/devices/system/cpu/cpu{cpu_id}/cpufreq"
    cmd  = (f"echo {freq_hz} > {path}/scaling_min_freq && "
            f"echo {freq_hz} > {path}/scaling_max_freq && "
            f"echo {freq_hz} > {path}/scaling_min_freq && "
            f"echo {freq_hz} > {path}/scaling_max_freq")
    subprocess.run(['adb', 'shell', cmd], capture_output=True, timeout=15)


def set_mem_freqs(ddr_hz, llcc_hz, ddrqos):
    """Lock DDR/LLCC via bwmon, DDRQOS via memlat prime. All chained to minimize ADB overhead."""
    _adb(
        f"echo {ddr_hz} > {DDR_BWMON}/max_freq && "
        f"echo {ddr_hz} > {DDR_BWMON}/min_freq && "
        f"echo {ddr_hz} > {DDR_BWMON}/max_freq && "
        f"echo {ddr_hz} > {DDR_BWMON}/min_freq && "
        f"echo {llcc_hz} > {LLCC_BWMON}/max_freq && "
        f"echo {llcc_hz} > {LLCC_BWMON}/min_freq && "
        f"echo {llcc_hz} > {LLCC_BWMON}/max_freq && "
        f"echo {llcc_hz} > {LLCC_BWMON}/min_freq"
    )
    _adb(
        f"echo {ddrqos} > {DDRQOS_PRIME}/max_freq && "
        f"echo {ddrqos} > {DDRQOS_PRIME}/min_freq && "
        f"echo {ddrqos} > {DDRQOS_PRIME}/max_freq && "
        f"echo {ddrqos} > {DDRQOS_PRIME}/min_freq && "
        f"echo {ddrqos} > {DDRQOS_BASE}/boost_freq"
    )


# ---------------------------------------------------------------------------
# Thermal wait
# ---------------------------------------------------------------------------
def wait_for_cool(cpu_id, threshold_c=DEFAULT_TEMP, poll_sec=3):
    zone_path = CPU_INFO[cpu_id]['thermal']
    waiting   = False
    while True:
        try:
            result = subprocess.run(
                ['adb', 'shell', f'cat {zone_path}'],
                capture_output=True, text=True, timeout=10)
            raw = result.stdout.strip()
            if raw.lstrip('-').isdigit():
                temp = int(raw) / 1000.0
                if temp <= threshold_c:
                    if waiting:
                        print(f"    [TEMP] cpu{cpu_id} {temp:.1f}C — OK")
                    return
                if not waiting:
                    print(f"    [TEMP] cpu{cpu_id} {temp:.1f}C > {threshold_c}C — cooling...")
                    waiting = True
            else:
                return
        except Exception:
            return
        time.sleep(poll_sec)


# ---------------------------------------------------------------------------
# Benchmark runner
# ---------------------------------------------------------------------------
def run_on_cpu(cpu_id, buffer_kb):
    mask = CPU_INFO[cpu_id]['mask']
    cmd  = (f"taskset {mask} /data/local/tmp/FullRandLat memlat "
            f"-min-buffer-size-kb {buffer_kb} -max-buffer-size-kb {buffer_kb}")
    try:
        result = subprocess.run(['adb', 'shell', cmd],
                                capture_output=True, text=True, timeout=120)
        match = re.search(r'\d+,(\d+\.\d+)', result.stdout + result.stderr)
        return float(match.group(1)) if match else None
    except Exception:
        return None


# ---------------------------------------------------------------------------
# Thompson Sampling Agent
# Prior: N(150, 30) — pessimistic prior so untested combos rarely beat the best
# ---------------------------------------------------------------------------
class ThompsonAgent:
    def __init__(self, n_actions, prior_mean=150.0, prior_std=30.0):
        self.n      = n_actions
        self.mu     = np.full(n_actions, prior_mean)
        self.sigma  = np.full(n_actions, prior_std)
        self.counts = np.zeros(n_actions)
        self.sum_x  = np.zeros(n_actions)
        self.sum_x2 = np.zeros(n_actions)

    def select(self):
        return int(np.argmin(np.random.normal(self.mu, self.sigma)))

    def update(self, a, latency):
        self.counts[a] += 1
        self.sum_x[a]  += latency
        self.sum_x2[a] += latency ** 2
        n = self.counts[a]
        self.mu[a] = self.sum_x[a] / n
        if n > 1:
            var = (self.sum_x2[a] - n * self.mu[a] ** 2) / (n - 1)
            self.sigma[a] = max(0.5, np.sqrt(var / n))
        else:
            self.sigma[a] = 20.0

    def best(self):
        tested = [i for i in range(self.n) if self.counts[i] > 0]
        return min(tested, key=lambda i: self.mu[i])

    def avg_latency(self, a):
        return self.mu[a] if self.counts[a] > 0 else None


# ---------------------------------------------------------------------------
# Periodic save (every 10 episodes) — preserves progress on interrupt
# ---------------------------------------------------------------------------
def _save_results_partial(agent, best_idx, history, buffer_kb, n_episodes, ep):
    rec_cpu, rec_cpu_freq, rec_ddr, rec_llcc, rec_qos = ACTIONS[best_idx]
    tried  = sum(1 for i in range(N_ACTIONS) if agent.counts[i] > 0)
    tested = sorted(
        [(i, agent.avg_latency(i), int(agent.counts[i]))
         for i in range(N_ACTIONS) if agent.counts[i] > 0],
        key=lambda x: x[1])
    results = {
        'status': f'running (ep {ep}/{n_episodes})',
        'recommended': {
            'cpu': rec_cpu, 'cluster': CPU_INFO[rec_cpu]['cluster'],
            'cpu_freq_hz': rec_cpu_freq, 'cpu_freq_mhz': rec_cpu_freq / 1000,
            'ddr_hz': rec_ddr, 'ddr_mhz': rec_ddr / 1000,
            'llcc_hz': rec_llcc, 'llcc_mhz': rec_llcc / 1000,
            'ddrqos': rec_qos,
            'taskset_mask': CPU_INFO[rec_cpu]['mask'],
        },
        'avg_latency_ns': float(agent.avg_latency(best_idx)),
        'explored': tried,
        'total_actions': N_ACTIONS,
        'buffer_kb': buffer_kb,
        'episodes_completed': ep,
        'episodes_total': n_episodes,
        'top_results': [
            {'cpu': ACTIONS[i][0], 'cpu_freq_hz': ACTIONS[i][1],
             'ddr_hz': ACTIONS[i][2], 'llcc_hz': ACTIONS[i][3], 'ddrqos': ACTIONS[i][4],
             'avg_latency_ns': float(lat), 'runs': runs}
            for i, lat, runs in tested[:20]
        ],
        'history': history
    }
    with open('rl_results.json', 'w') as f:
        json.dump(results, f, indent=2)


# ---------------------------------------------------------------------------
# Training loop
# ---------------------------------------------------------------------------
def train(n_episodes, buffer_kb):
    agent = ThompsonAgent(N_ACTIONS)
    setup_device()

    print("\n" + "=" * 80)
    print("  RL Joint Optimizer: CPU x CPU_freq x DDR x LLCC x DDRQOS")
    print("=" * 80)
    print(f"  Agent    : THOMPSON SAMPLING  (prior N(150, 30))")
    print(f"  Actions  : {N_ACTIONS:,}  (241 CPU×freq  ×  11 DDR  ×  9 LLCC  ×  2 DDRQOS)")
    print(f"  Episodes : {n_episodes}")
    print(f"  Buffer   : {buffer_kb} KB")
    print(f"  Thermal  : wait <= {DEFAULT_TEMP}C before each run")
    print("=" * 80)
    print(f"  {'Ep':>4}  {'CPU':>4}  {'CPU MHz':>8}  {'DDR MHz':>8}  {'LLCC MHz':>9}  "
          f"{'QOS':>4}  {'Latency':>10}  {'Best so far'}")
    print("  " + "-" * 78)

    history  = []
    best_lat = float('inf')
    best_idx = None

    for ep in range(1, n_episodes + 1):
        idx                                        = agent.select()
        cpu_id, cpu_freq, ddr_hz, llcc_hz, ddrqos = ACTIONS[idx]
        cluster                                    = CPU_INFO[cpu_id]['cluster']

        set_cpu_freq(cpu_id, cpu_freq)
        set_mem_freqs(ddr_hz, llcc_hz, ddrqos)
        wait_for_cool(cpu_id)
        latency = run_on_cpu(cpu_id, buffer_kb)

        if latency is not None:
            agent.update(idx, latency)
            if latency < best_lat:
                best_lat = latency
                best_idx = idx

        lat_str  = f"{latency:.2f} ns" if latency else "FAILED"
        if best_idx is not None:
            b = ACTIONS[best_idx]
            best_str = (f"cpu{b[0]}@{b[1]/1000:.0f} "
                        f"DDR{b[2]/1000:.0f} "
                        f"LLCC{b[3]/1000:.0f} "
                        f"QOS{b[4]} "
                        f"= {best_lat:.1f}ns")
        else:
            best_str = "-"

        print(f"  {ep:>4}  cpu{cpu_id}  {cpu_freq/1000:>6.1f}  {ddr_hz/1000:>6.0f}  "
              f"{llcc_hz/1000:>7.0f}  {ddrqos:>4}  {lat_str:>10}  {best_str}")

        history.append({
            'episode': ep, 'cpu': cpu_id, 'cluster': cluster,
            'cpu_freq_hz': cpu_freq, 'ddr_hz': ddr_hz,
            'llcc_hz': llcc_hz, 'ddrqos': ddrqos, 'latency_ns': latency
        })

        if best_idx is not None and ep % 10 == 0:
            _save_results_partial(agent, best_idx, history, buffer_kb, n_episodes, ep)

    # -----------------------------------------------------------------------
    # Final recommendation
    # -----------------------------------------------------------------------
    rec_idx                                        = agent.best()
    rec_cpu, rec_cpu_freq, rec_ddr, rec_llcc, rec_qos = ACTIONS[rec_idx]
    rec_cluster                                    = CPU_INFO[rec_cpu]['cluster']
    tried                                          = sum(1 for i in range(N_ACTIONS) if agent.counts[i] > 0)

    print("\n" + "=" * 80)
    print("  RECOMMENDATION")
    print("=" * 80)
    print(f"  CPU          : cpu{rec_cpu}  ({rec_cluster} cluster)")
    print(f"  CPU freq     : {rec_cpu_freq:,} Hz  ({rec_cpu_freq/1000:.1f} MHz)")
    print(f"  DDR freq     : {rec_ddr:,} Hz  ({rec_ddr/1000:.0f} MHz)")
    print(f"  LLCC freq    : {rec_llcc:,} Hz  ({rec_llcc/1000:.0f} MHz)")
    print(f"  DDRQOS       : {rec_qos}")
    print(f"  Avg latency  : {agent.avg_latency(rec_idx):.2f} ns")
    print(f"  Explored     : {tried:,}/{N_ACTIONS:,} combinations ({tried*100//N_ACTIONS}%)")
    print()

    tested = sorted(
        [(i, agent.avg_latency(i), int(agent.counts[i]))
         for i in range(N_ACTIONS) if agent.counts[i] > 0],
        key=lambda x: x[1])

    print(f"  {'CPU':<5} {'CPU MHz':>8}  {'DDR MHz':>8}  {'LLCC MHz':>9}  {'QOS':>4}  "
          f"{'Avg Latency':>12}  {'Runs':>5}")
    print("  " + "-" * 65)
    for i, lat, runs in tested[:15]:
        a   = ACTIONS[i]
        tag = " <-- BEST" if i == rec_idx else ""
        print(f"  cpu{a[0]}  {a[1]/1000:>6.1f}  {a[2]/1000:>6.0f}  {a[3]/1000:>7.0f}  "
              f"{a[4]:>4}  {lat:>10.2f} ns  {runs:>5}{tag}")

    print()
    print("  Apply this configuration (hard lock via bwmon):")
    print(f"  adb shell \"echo {rec_cpu_freq} > /sys/devices/system/cpu/cpu{rec_cpu}/cpufreq/scaling_min_freq && "
          f"echo {rec_cpu_freq} > /sys/devices/system/cpu/cpu{rec_cpu}/cpufreq/scaling_max_freq\"")
    print(f"  adb shell \"echo {rec_ddr} > {DDR_BWMON}/max_freq && echo {rec_ddr} > {DDR_BWMON}/min_freq\"")
    print(f"  adb shell \"echo {rec_llcc} > {LLCC_BWMON}/max_freq && echo {rec_llcc} > {LLCC_BWMON}/min_freq\"")
    print(f"  adb shell \"echo {rec_qos} > {DDRQOS_PRIME}/max_freq && echo {rec_qos} > {DDRQOS_PRIME}/min_freq\"")
    print(f"  adb shell taskset {CPU_INFO[rec_cpu]['mask']} "
          f"/data/local/tmp/FullRandLat memlat "
          f"-min-buffer-size-kb {buffer_kb} -max-buffer-size-kb {buffer_kb}")
    print("=" * 80)

    results = {
        'recommended': {
            'cpu': rec_cpu, 'cluster': rec_cluster,
            'cpu_freq_hz': rec_cpu_freq, 'cpu_freq_mhz': rec_cpu_freq / 1000,
            'ddr_hz': rec_ddr, 'ddr_mhz': rec_ddr / 1000,
            'llcc_hz': rec_llcc, 'llcc_mhz': rec_llcc / 1000,
            'ddrqos': rec_qos,
            'taskset_mask': CPU_INFO[rec_cpu]['mask'],
        },
        'avg_latency_ns': float(agent.avg_latency(rec_idx)),
        'explored': tried,
        'total_actions': N_ACTIONS,
        'buffer_kb': buffer_kb,
        'episodes': n_episodes,
        'top_results': [
            {'cpu': ACTIONS[i][0], 'cpu_freq_hz': ACTIONS[i][1],
             'ddr_hz': ACTIONS[i][2], 'llcc_hz': ACTIONS[i][3], 'ddrqos': ACTIONS[i][4],
             'avg_latency_ns': float(lat), 'runs': runs}
            for i, lat, runs in tested[:20]
        ],
        'history': history
    }
    with open('rl_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print("  Saved -> rl_results.json")
    return results['recommended']


# ---------------------------------------------------------------------------
# Entry point
# ---------------------------------------------------------------------------
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='RL: find best (CPU, CPU_freq, DDR, LLCC, DDRQOS) for min DRAM latency'
    )
    parser.add_argument('--episodes', type=int, default=500,
                        help='Total runs (default 500; action space = 47,718)')
    parser.add_argument('--buffer-kb', type=int, default=DEFAULT_BUF,
                        help='Buffer size in KB (default 40960 = 40MB)')
    args = parser.parse_args()

    train(args.episodes, args.buffer_kb)