"""
Run best configuration 10 times with thermal check between each run.
cpu6 @ 4435.2 MHz, DDR=4224 MHz, LLCC=806 MHz, QOS=1
Buffer: 65536 KB (64 MB)
Same conditions as rl_benchmark_cpu6.py
"""
...
def setup():
    ...
    print("[STEP 4] Silencing all bus_dcvs nodes to hw_min...")
    ...
    print("[STEP 5] Applying best configuration...")
    ...
    print(f"  cpu6={CPU_FREQ/1000:.1f}MHz  DDR={DDR/1000:.0f}MHz  "
          f"LLCC={LLCC/1000:.0f}MHz  QOS={QOS}")
...
def main():
    ...
    print()
    print("=" * 60)
    print(f"  Running {RUNS} times with thermal check (≤{TEMP_MAX}°C)")
    print("=" * 60)
    ...
    if results:
        print()
        print("=" * 60)
        print(f"  RESULTS ({len(results)}/{RUNS} successful)")
